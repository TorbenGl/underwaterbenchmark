...
@register_model
class UperNetFrancaBaseLastLayer(UperNetFrancaBase):
    """UperNet with Franca ViT-B/14 backbone — last-layer-only features."""
    name = "upernet_franca_base_lastlayer"
    backbone_indices = [11, 11, 11, 11]


@register_model
class UperNetFrancaLargeLAIONLastLayer(UperNetFrancaLargeLAION):
    """UperNet with Franca ViT-L/14 (LAION) backbone — last-layer-only features."""
    name = "upernet_franca_large_laion_lastlayer"
    backbone_indices = [23, 23, 23, 23]


@register_model
class UperNetFrancaGiantLAIONLastLayer(UperNetFrancaGiantLAION):
    """UperNet with Franca ViT-g/14 (LAION) backbone — last-layer-only features."""
    name = "upernet_franca_giant_laion_lastlayer"
    backbone_indices = [39, 39, 39, 39]


@register_model
class UperNetFrancaBaseNoRASALastLayer(UperNetFrancaBaseNoRASA):
    """UperNet with Franca ViT-B/14 without RASA — last-layer-only features."""
    name = "upernet_franca_base_no_rasa_lastlayer"
    backbone_indices = [11, 11, 11, 11]


@register_model
class UperNetFrancaLargeNoRASALastLayer(UperNetFrancaLargeNoRASA):
    """UperNet with Franca ViT-L/14 (LAION) without RASA — last-layer-only features."""
    name = "upernet_franca_large_no_rasa_lastlayer"
    backbone_indices = [23, 23, 23, 23]


@register_model
class UperNetFrancaGiantLAIONNoRASALastLayer(UperNetFrancaGiantLAIONNoRASA):
    """UperNet with Franca ViT-g/14 (LAION) without RASA — last-layer-only features."""
    name = "upernet_franca_giant_no_rasa_lastlayer"
    backbone_indices = [39, 39, 39, 39]
