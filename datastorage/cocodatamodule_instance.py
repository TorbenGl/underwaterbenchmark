...
from vfm_benchmark.underwaterbenchmark.datastorage.cocodataset_instance import COCODataset_Instance
import torchvision.transforms.v2 as transforms
from datastorage.transforms import RandomResizeCropSeg

class CocoLightningDataModule_Instance(lightning.LightningDataModule):
    def __init__(
        self,
        path,
        image_folder,
        ...
        super().__init__()        
        self.path = path
        self.ignore_idx = ignore_idx # Ignore Background when generating masks for mask2former
        ...
        self.val_dataset = COCODataset_Instance(path=self.path,
                                       annotation_file=self.annotation_file_dict["val"],
                                       image_folder=self.image_folder,                                       
                                       id2label=self.id2label,
                                        increment_classes=increment_classes )

        self.test_dataset = COCODataset_Instance(path=self.path,
    ...
    def collate_fn(self,batch):
        ...
        instanceid_to_classid=inputs[2]
        batch = self.preprocessor(
            images,
            segmentation_maps=segmentation_maps,
            instanceid_to_classid=instanceid_to_classid,
            size=self.img_size,
            return_tensors="pt",
