...
Usage:
    python train_semantic.py --dataset cou --model upernet_vit_base
    python train_semantic.py --dataset suim --model upernet_dinov2_base --batch_size 8

Available models:
    upernet_vit_base, upernet_vit_large, upernet_dinov2_base, upernet_dinov2_large,
    upernet_dinov2_giant, upernet_dinov2_base_reg, upernet_dinov2_large_reg,
    upernet_beit_base, upernet_beit_large
"""
import argparse
...
from modules.SemanticLightningModule import SemanticLightningModule
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default="upernet_vit_base", choices=AVAILABLE_MODELS,
...
def print_config(args, datamodule, model_info):
    ...
    if RICH_AVAILABLE:
        ...
        table.add_row("Backbone", model_info["backbone"])
...
def main():
    ...
    # Setup logger and run directory (do this early to get run_dir)
    ...
    # Get dataset metadata without full instantiation
    ...
    loss_ignore_index = metadata.get("loss_ignore_index", 255)
    metric_ignore_index = metadata.get("metric_ignore_index", 0)
    ...
    preprocessor = PaddingPreprocessor(
        ...
        mask_pad_value=loss_ignore_index,
    )
    ...
    lightning_module = SemanticLightningModule(
        model=model,
        num_classes=num_classes,
        learning_rate=args.lr,
        weight_decay=args.weight_decay,
        loss_ignore_index=loss_ignore_index,
        metrics_ignore_index=metric_ignore_index,
    ...
    # Setup callbacks (logger already created earlier)
    ...
    print(f"Best val/iou: {callbacks[0].best_model_score:.4f}")
